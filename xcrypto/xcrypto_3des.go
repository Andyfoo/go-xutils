...
func TripleDesEncrypt(src []byte, key []byte, iv []byte) []byte {
	if len(key) != 24 {
		xlog.Error("key size must is 24")
		return []byte{}
	}
	if len(iv) != 8 {
...
func TripleDesEncryptHex(src string, key string, iv string) string {
	return xencode.HexEncodeStr(string(TripleDesEncrypt([]byte(src), []byte(key), []byte(iv))))
}
func TripleDesEncryptBase64(src string, key string, iv string) string {
	return xencode.Base64EncodeStr(string(TripleDesEncrypt([]byte(src), []byte(key), []byte(iv))))
}
...
func TripleDesDecryptHex(src string, key string, iv string) string {
	return string(TripleDesDecrypt(xencode.HexDecode([]byte(src)), []byte(key), []byte(iv)))
}

func TripleDesDecryptBase64(src string, key string, iv string) string {
	return string(TripleDesDecrypt(xencode.Base64Decode([]byte(src)), []byte(key), []byte(iv)))
}
